...
interface DataTableProps {
	users: DataUser[]; // Array of user data
...
const DataTable: React.FC<DataTableProps> = ({ users: initialUsers, sessionToken }) => {
	...
	return (
		<div className='w-full h-screen flex flex-col gap-y-4'>
						...
						{paginatedData.length > 0 ? (
							paginatedData.map((user) => (
								<TableRow key={user.id}>
									...
									{visibleKeys.map((key) => (
										<TableCell key={`${user.id}-${key}`}>{user[key as keyof DataUser] || 'N/A'}</TableCell>
									))}
			...
			<DataTablePagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} pageSize={pageSize} totalItems={filteredUsers.length} disabled={false} />
		</div>
	);
};
