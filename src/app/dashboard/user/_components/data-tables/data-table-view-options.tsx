...
function DataTableViewOptions({ allColumnHeaders = [], visibleKeys, onViewOptionChange }: DataTableViewOptionsProps) {
	...
	const handleCheckedChange = (header: string, isChecked: boolean) => {
		...
		if (isChecked && selectedCount >= 5) {
			...
			setAlertMessage('Karena untuk user experience maksimal data yang ditampilkan adalah 5, maka Anda harus menghilangkan centang pada salah satu data terlebih dahulu sebelum dapat menampilkan data yang diinginkan.');
			setAlertOpen(true);
							...
							{allColumnHeaders.length > 0 ? (
								allColumnHeaders.map((header) => (
									<div key={header} className='flex items-center'>
										<Checkbox checked={checkedState[header]} onCheckedChange={(isChecked) => handleCheckedChange(header, Boolean(isChecked))} /> <span className='ml-2'>{header}</span>
									</div>
								))
							) : (
