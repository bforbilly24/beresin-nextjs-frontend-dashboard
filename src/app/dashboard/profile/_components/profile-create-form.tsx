...
const ProfileCreateForm: React.FC<ProfileFormType> = ({ initialData, categories }) => {
	...
	const title = initialData ? 'Edit service' : 'Create Your Profile';
	const description = initialData ? 'Edit a service.' : 'To create your resume, we first need some basic information about you.';
	...
	const onSubmit = async (data: ProfileFormValues) => {
		try {
			setLoading(true);
			if (initialData) {
				// await axios.post(`/api/services/edit-service/${initialData._id}`, data);
			} else {
				// const res = await axios.post(`/api/services/create-service`, data);
	...
	const onDelete = async () => {
		try {
			setLoading(true);
			//   await axios.delete(`/api/${params.storeId}/services/${params.serviceId}`);
			router.refresh();
								...
								{fields?.map((field, index) => (
									<Accordion type='single' collapsible defaultValue='item-1' key={field.id}>
										<AccordionItem value='item-1'>
											<AccordionTrigger className={cn('relative !no-underline [&[data-state=closed]>button]:hidden [&[data-state=open]>.alert]:hidden', errors?.jobs?.[index] && 'text-red-700')}>
